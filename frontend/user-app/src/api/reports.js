...
const reportsAPI = {
  ...
  getMyReports: async (status = 'all') => {
    try {
      // Always use /me endpoint explicitly
      const url = '/api/reports/me' + (status !== 'all' ? `?status=${status.toUpperCase()}` : '');
      
      console.log('Fetching reports from URL:', url); // Log URL for debugging
  ...
  getReportCategories: async () => {
    try {
      // This route might not exist yet - could be added to backend later
